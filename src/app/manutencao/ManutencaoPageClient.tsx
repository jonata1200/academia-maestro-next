...
import { Container, Button, Card } from '@/design-system/components';
import { UNITS, whatsappUrl } from '@/lib/site';

export default function ManutencaoPageClient() {
  const whatsappLink = whatsappUrl(
    UNITS[0].whatsapp,
    'Olá! Vi no site que vocês fazem conserto e manutenção de instrumentos e gostaria de um orçamento.'
  );

  return (
    <>
      {/* Hero Section */}
      <section className="relative text-white text-center flex flex-col items-center justify-center min-h-[400px] px-5 isolate md:pt-[220px] md:pb-[120px] md:px-5 md:flex-none md:min-h-0">
        <Image
          src="/manutencao-bg.jpg"
          alt=""
          fill
          priority
          sizes="100vw"
        ...
        <Container>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <Card variant="outlined" className="text-center transition-all duration-300 border-t-[3px] border-t-transparent hover:-translate-y-2.5 hover:shadow-[0_12px_25px_rgba(0,0,0,0.08)] hover:border-t-primary group p-8">
              <div className="text-4xl w-16 h-16 mx-auto mb-5 bg-primary/10 rounded-full flex justify-center items-center">
                <span>🛠️</span>
              </div>
